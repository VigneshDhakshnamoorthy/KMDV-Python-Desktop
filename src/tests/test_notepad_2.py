...
find_what: WindowSpecification = replace.child_window(
    title="Find what:", control_type="Edit"
)
find_what.type_keys("^a{DEL}")
find_what.type_keys("World", with_spaces=True)

replace_with: WindowSpecification = replace.child_window(
    title="Replace with:", control_type="Edit"
)
replace_with.type_keys("^a{DEL}")
replace_with.type_keys("CES", with_spaces=True)


replace_all_button: WindowSpecification = replace.child_window(
    title="Replace All", control_type="Button"
)
replace_all_button.click()


cancel_button: WindowSpecification = replace.child_window(
    title="Cancel", control_type="Button"
)
cancel_button.click()

close_button: WindowSpecification = notpad.child_window(
...
cancel_save_changes_button: WindowSpecification = save_changes_window.child_window(
    title="Cancel", control_type="Button"
)
...
drag_scroll_bar(scroll_bar_rect, direction="down", distance=5000)

time.sleep(2)
# Drag the scroll bar up
drag_scroll_bar(scroll_bar_rect, direction="up", distance=1000)

time.sleep(2)

notpad.menu_select("File -> Exit")

dont_save_changes_button: WindowSpecification = save_changes_window.child_window(
